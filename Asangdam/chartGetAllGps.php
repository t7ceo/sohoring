	...
	if(!$rr){
		...
		$i = 1;
       	while($row=mysql_fetch_array($rr)){
			...
			//열려있는 방의 수와 위도 경도를 가져온다.
			$oprr = mysql_query("select count(a.companyid) as comid, b.latpo, b.longpo from Anyting_master as a left join Anyting_comgps as b on(a.companyid = b.companyid) where a.companyid = '".$row[companyid]."' and a.roomInf = 1",$rs);			if(!$oprr){
			...
			$jsongab .= '{"comid":"'.$row[companyid].'", "comimg":"'.$imgrow[imgname].'", "memo":"'.$row[memo].'", "tel":"'.$row[tel].'", "sangho":"'.$row[sangho].'", "url":"'.$row[url].'", "addr":"'.$row[juso].'", "tbsu":'.$imgrow[tablesu].', "rsu":'.$roomsu[comid].', "latpo":'.$roomsu[latpo].' , "longpo":'.$roomsu[longpo].' }';
